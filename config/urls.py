...
urlpatterns += [
    # API base url
    path("backend/api/", include("config.api_router")),
    # DRF auth token
    path("backend/auth-token/", obtain_auth_token),
    path("backend/login/", CustomAuthToken.as_view()),
    
    path("backend/api/schema/", SpectacularAPIView.as_view(), name="api-schema"),
    path(
        "backend/api/docs/",
